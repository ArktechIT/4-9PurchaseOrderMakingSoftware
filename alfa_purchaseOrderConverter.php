<?php
	...
	$previewFlag2 = (isset($_GET['preview2']) AND $_GET['preview2']==0) ? 0 : 1;
	...
	//~ if(in_array($poNumber,array('0015717','0015675','0015694')))	$previewFlag = 0;
	//~ if(in_array($poNumber,array('0015702','0015703')))	$previewFlag = 0;
	//~ if(in_array($poNumber,array('0012109','0012055','0012098','0011975','0012047','0012074','0011999')))	$previewFlag = 0;
	//~ if(in_array($poNumber,array('0015628')))	$previewFlag = 0;
	...
	if($previewFlag2==0)	$previewFlag = 0;
	
	if($previewFlag == 1)
	{
		$pdf->Image($path.'Templates/images/po1b.jpg',0,0,$pdf->w, $pdf->h);
	...
	//~ $pdf->SetXY(15.00125,61.00125);
	$pdf->SetXY(15.00125,58.00125);
	...
	$pdf->SetXY(120.00125,70.00125);
	$pdf->MultiCell(79,5,$supplierContactPerson,0);
	...
	$pdf->SetXY(41.00125,93.00125);
	
	$pdf->MultiCell(37,3,$deliveryDate,0);
	$pdf->SetXY(80.00125,93.00125);
	$pdf->Cell(54,5,$shipping,0,0,'C');
	$pdf->SetXY(136.00125,93.00125);
	$pdf->Cell(63,5,$firstName." ".$surName,0,0,'C');
	$pdf->SetXY(41.00125,107.00125);
							...
							while($resultPoContents = $queryPoContents->fetch_assoc())
							{
								...
								if($supplierType==1)
								{
									...
									else
									{
										...
										$partialReceivingArray[] = "Receiving: ".$receivingDate;
									}
								}
								else if($supplierType==2)
								{
			...
			if(count($chargeDescriptionArray) > 0)
			{
				foreach($chargeDescriptionArray as $key => $val)
				{
					...
					{				
						...
						if($_GET['country']==2)	$pdf->SetFont('SJIS','',10);
						$pdf->Cell(14,5,$unitName,0,0,'L');
						$pdf->Cell(96,5,$description[0],0,0,'L');
						if($_GET['country']==2)	$pdf->SetFont('Arial','',10);
						$pdf->Cell(31,5,$priceInFormat,0,0,'R');
						$pdf->Cell(1,5,'',0);
						$pdf->Cell(29,5,$totalPriceInFormat,0,0,'R');
						...
						$pdf->Cell(14,4,"",0,0,'L');
						if($_GET['country']==2)	$pdf->SetFont('SJIS','',10);
						$pdf->MultiCell(96,4,$description[1],0,'L');
						if($_GET['country']==2)	$pdf->SetFont('Arial','',10);
					}
					else
					{
	...
	if($seeAttachFlag == 1)
	{
		...
		$pdf->Cell(190,5,"SEE ATTACHED",0,0,'C');
	}
	else
	{
		$pdf->SetFont('Arial','',10);//Bold
	...
	$pdf->SetXY(171.00125,237.00125);
	$pdf->Cell(29,5,$totalAmountInFormat,0,0,'R');
	
	$poDiscountInFormat = '';
	if($poDiscount > 0)
	{
		$pdf->SetXY(151.00125,254.00125);
	...
	$pdf->SetXY(171.00125,254.00125);
	$pdf->Cell(29,5,$poDiscountInFormat,0,0,'R');
	
	$totalAmountInFormat = ($totalAmount > 0) ? $sign." ".number_format($totalAmount, 2, '.', ',') : '';
	
	$pdf->SetXY(171.00125,259.00125);
	...
	if($approvedBy != '')
	{
		...
		if(isset($poInputDateTime))//2018-06-23 by sir Nagano all BE items to KAPCO
		{
			if($signatureFlag==1)
			{
				if(strtotime($poInputDateTime) >= strtotime('2018-06-23'))
				{
					$pdf->Image($path.'Templates/images/nagano.jpg',104,263,28,10);
				}
			}
		}
		//if(strtotime($poInputDateTime) >= strtotime('2021-06-14 17:51:00'))
        if(strtotime($poInputDateTime) >= strtotime('2021-06-14 11:00:00'))
		{
			$vp = '';
		}
		else if(strtotime($poInputDateTime) >= strtotime('2019-09-12 11:04:00'))
		{
			$vp = 'KAZUMASA NAGANO';
		}
		else
	...
	$pdf->SetXY(10.00125,270.00125);
	$pdf->Cell(60,5,strtoupper($checked),0,0,'C');
	
	$pdf->SetXY(80.00125,270.00125);
	$pdf->Cell(60,5,$vp,0,0,'C');
	
	$pdf->SetXY(141.00125,270.00125);
	$pdf->Cell(60,5,$president,0,0,'C');
	...
	if($saveFileFlag==1 AND $previewFlag==1)
	{
		...
		$attachmentFile = $_SERVER['DOCUMENT_ROOT']."/".v."/4-9 Purchase Order Making Software/AutoEmail/".$poNumber."-1.pdf";
		$pdf->Output($attachmentFile, 'F');
		
		header('location:alfa_invoiceDetailsConverter.php?saveFile=1&poNumber='.$poNumber);
	}
	else
	{
		if($downloadFlag == 1)
