<?php
	...
	if(isset($_GET['choicesFlag']))
	{
		...
		else
		{
			...
			if($queryPoContents AND $queryPoContents->num_rows > 0)
			{
				while($resultPoContents = $queryPoContents->fetch_assoc())
				{
					...
					$sql = "SELECT lotNumber, workingQuantity FROM ppic_lotlist WHERE poContentId LIKE '".$poContentIds."' AND workingQuantity > 0 AND identifier = 1";
					$queryLotList = $db->query($sql);
					if($queryLotList AND $queryLotList->num_rows > 0)
					{
						while($resultLotList = $queryLotList->fetch_assoc())
		...
		echo "
			<tr>
				<th colspan='3'><input type='submit' form='itemTagFormId'></th>
			</tr>
		";
		echo "</table>";
		?>
	...
	if($saveFileFlag==1)
	{
		$attachmentFile = $_SERVER['DOCUMENT_ROOT']."/".v."/4-9 Purchase Order Making Software/AutoEmail/".$poNumber."-2.pdf";
		$pdf->Output($attachmentFile, 'F');
		
		header('location:alfa_mergeEmailAttachment.php?poNumber='.$poNumber);
	}
	else
	{
		$pdf->Output();
