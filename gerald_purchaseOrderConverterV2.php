<?php
	...
	start:
	$previewFlag2 = (isset($_GET['preview']) AND $_GET['preview']==0) ? 0 : 1;
	$previewFlag = (isset($_POST['preview']) AND $_POST['preview']==0) ? 0 : $previewFlag2;
	...
	{
		$poNumber = $poNo;
		$sql = "SELECT supplierId, supplierType, poTerms, poShipmentType, poIncharge, poIssueDate, poTargetReceiveDate, poRemarks, poCurrency, poDiscount, poInputDateTime, checkedBy, approvedBy, vatFlag FROM purchasing_podetailsnew WHERE poNumber LIKE '".$poNumber."' LIMIT 1";
		...
		if($queryPodetailsNew AND $queryPodetailsNew->num_rows > 0)
		{
			...
			$vatFlag = $resultPodetailsNew['vatFlag'];
		}
		
		$lotNumberArray = array();
		$sql = "SELECT DISTINCT lotNumber FROM purchasing_pocontents WHERE poNumber LIKE '".$poNumber."'";
		if($_SESSION['idNumber']=='0346') $sql = "SELECT DISTINCT lotNumber FROM purchasing_pocontents WHERE poNumber LIKE '".$poNumber."' ORDER BY orderNo, poContentId";
		if($poNumber=='0016675') $sql = "SELECT DISTINCT lotNumber FROM purchasing_pocontents WHERE poNumber LIKE '".$poNumber."' ORDER BY orderNo, poContentId";// by rose 2021-12-02
		$queryPoContents = $db->query($sql);
	...
	{
		//~ if($poNo=='0016228')	$poNumber = '0016228-1';
		//~ if($poNo=='0016256')	$poNumber = '0016228-2';
	}
	
	if($poNumber=='0015438')	$poRemarks = '';
	...
	//~ if(in_array($poNumber,array('0015954','0015947','0015946')))	$previewFlag = 0;
	//~ if(in_array($poNumber,array('0015985','0015769','0015772','0015966','0015958','0015961','0015913','0015948','0015920','0015919')))	$previewFlag = 0;
	//~ if(in_array($poNumber,array('0012109','0012055','0012098','0011975','0012047','0012074','0011999')))	$previewFlag = 0;
	//~ if(in_array($poNumber,array('0015992')))	$previewFlag = 0;
	...
	if($supplierType==1)
	{
		...
		if($poNumber=='0015605')	$deliveryDate = 'To be discuss';
		if($poNumber=='0015607')	$deliveryDate = 'To be discuss';
		if($poNumber=='0013837')	$deliveryDate = 'ASAP';
		if($poNumber=='0015329')	$deliveryDate = 'To be advised';
		if($poNumber=='0015333')	$deliveryDate = 'To be advised';
		if($poNumber=='0015658')	$deliveryDate = 'To be advised';
		# RG 2019-01-11
	...
	if($queryPoSpecialView AND $queryPoSpecialView->num_rows > 0)
	{
		while($resultPoSpecialView = $queryPoSpecialView->fetch_assoc())
		{
			...
			*/
			...
			$pdf->SetFont('Arial','',10);//Bold
			$pdf->Cell(12,5,$item,0,0,'L');
			$pdf->Cell(7,5,$quantity,0,0,'R');
			if($_GET['country']==2)	$pdf->SetFont('SJIS','',10);
			$pdf->Cell(14,5,$unitName,0,0,'L');
			$pdf->Cell(92,5,$description,0,0,'L');
			if($_GET['country']==2)	$pdf->SetFont('Arial','',10);
		...
		if($poNumber=='0015900') goto jumphere;
	}
	else
	{
		jumphere:
		...
		if(count($lotNumberArray) > 0)
		{
			...
			foreach($lotNumberArray as $lotNumber)
			{
				...
				if($queryLotList->num_rows > 0)
				{
					...
					if($poNo!='')
					{
						...
						if($queryPoContents AND $queryPoContents->num_rows > 0)
						{
							while($resultPoContents = $queryPoContents->fetch_assoc())
							{
								...
								$receivingDate = $resultPoContents['receivingDate'];
								...
								if($poNumber=='0015900')
								{
									$priceInFormat = $totalPriceInFormat = '';
								}
								...
								if($supplierType==1)
								{
									...
									else
									{
										...
										$partialReceivingArray[] = ($receivingDate=='0000-00-00') ? "" : "Receiving: ".$receivingDate;
									}
								}
								else if($supplierType==2)
								{
									...
									if(trim($treatmentProcess)=='MIL-8625 TYPE II CLASS 1 (MIN 9um)') $treatmentProcess = 'MIL-8625 Type II Class 1';
									$treatmentProcessCut = (strlen($treatmentProcess) > 15) ? substr($treatmentProcess,0,23)."..." : $treatmentProcess ;
									
									//~ if($_SESSION['idNumber']=='0346')
									//~ if($_SESSION['idNumber']==true)
									//~ {
										//~ $treatmentProcessTemp = $itemName;
										//~ $stringWidth = $pdf->GetStringWidth($treatmentProcessTemp);
										//~ $cellWidth = 37;
										//~ while($stringWidth > $cellWidth)
										//~ {
											//~ $treatmentProcessTemp = substr($treatmentProcessTemp, 0, -1);
											//~ $stringWidth = $pdf->GetStringWidth($treatmentProcessTemp);
										//~ }
										//~ if($treatmentProcessTemp!=$itemName)	$treatmentProcessTemp .= "...";
										//~ $treatmentProcessCut = $treatmentProcessTemp;
									//~ }
									...
									if($queryPoContentDetails AND $queryPoContentDetails->num_rows > 0)
									{
										...
										$receivingDate = ($resultPoContentDetails['receivingDate']=='0000-00-00') ? "" : "Receiving: ".$resultPoContentDetails['receivingDate'];
										$sendingDate = ($resultPoContentDetails['sendingDate']=='0000-00-00') ? "" : "Sending: ".$resultPoContentDetails['sendingDate'];
					...
					if($poNumber=='0016898')
					{
						if($itemUnit==3)	$unitName = 'Truck';
						else if($itemUnit==1)	$unitName = 'Day';
					}
					...
					if($identifier==4 AND $supplyType==1 AND (strtotime($poInputDateTime) >= strtotime('2019-12-17') OR $poInputDateTime==''))
					{
						if(in_array($supplierId,array(765,898,80,937)))
						{
							...
							if($partId==533)
							{
								$itemDescription = $itemName." ".$aeroMaterialSpecs." \n".$itemDescription;
								$itemName = $aeroMaterialRemarks." ".$aeroMaterialCode;
							}
							else
							{
					...
					if($seeAttachFlag == 0)
					{
						...
						if($seeAttachFlag == 0)
						{
							...
							else
							{
								...
								if(($currentY+$multiCellHeight) >= 230)
								{
									$seeAttachFlag = 1;
									goto start;
								}
	...
	else
	{
		...
		else if($poNumber=='0016898')
		{
			$pdf->Cell(38,5,"",0,0,'C');//190
			$pdf->MultiCell(154,6,$poRemarks,0,'L');
		}
		else
		{
	...
	if($vatFlag==1)
	{
		$totalVat = $totalAmount * .12;
		
		$totalAmount += $totalVat;
		
		$pdf->SetXY(167.00125,242.00125);
		$totalVatFormat = $sign." ".number_format($totalVat, 2, '.', ',');
		$pdf->Cell(29,5,$totalVatFormat,0,0,'R');
	}
	...
	if($checkedBy != '')
	{
		...
		*/
		...
		if($_SESSION['idNumber']=='0346' AND $poNumber=='0016615')
		{
			$pdf->Image($path.'Templates/images/sirAriel.jpg',32,266,28,8);
		}
	}
	...
	if($approvedBy != '')
	{
		...
		if(isset($poInputDateTime))//2018-06-23 by sir Nagano all BE items to KAPCO
		{
			if($signatureFlag==1)
			{
				...
				{
					//$pdf->Image($path.'Templates/images/shachou.jpg',104,263,28,10);
				}
		...
		if($_SESSION['idNumber']=='0346' AND $poNumber=='0016615')
		{
			$pdf->Image($path.'Templates/images/shachou.jpg',104,266,30,8);
		}
	}
	...
	if($seeAttachFlag==1 AND $previewFlag==1)
	{
		...
		if(count($contentDataArray) > 0)
		{
			...
			if(in_array($poNumber,array('0016034','0016794')))
			{
				$poRemarks = str_replace("Delivery Date:","",$poRemarks);
				$pdf->MultiCell(160,5,"Delivery Date:",0,'C');
				$poRemarksArray = explode("Item #",$poRemarks);
				if(count($poRemarksArray) > 0)
				{
					$poRemarks1Array = $poRemarks2Array = array();
					foreach($poRemarksArray as $key=> $poRemarks)
					{
						if(($key%2)==0)
						{
							$poRemarks2Array[] = $poRemarks;
						}
						else
						{
							$poRemarks1Array[] = $poRemarks;
						}
					}
					$pdf->SetXY(40,$currentY+5);
					$pdf->MultiCell((154/3),5,"\nItem #".implode("Item #",$poRemarks1Array),0,'C');
					$pdf->SetXY(90,$currentY+5);
					$pdf->MultiCell((154/3),5,implode("Item #",$poRemarks2Array),0,'C');
					$pdf->Ln(20);
				}
			}			
			else
