<?php
	...
	// --------------------------------------------------------------------------------
		...
		function Footer()
		{
			if($this->footer!=1)
			{
				$this->SetY(-7.5);
				$this->SetFont('SJIS','',9);
				//~ $this->Cell(0,5,'	”õl (‰¼)..‰¼’P‰¿ / (–])..Šó–]“WŠJ / (Œˆ)..Œˆ’è’P‰¿ ','T',0,'L');
				$this->Cell(0,5,'','T',0,'L');
			}
		}
		function Header(){
			...
			if($this->header!=1)
			{
				...
				$this->Cell(7,10,'',0,0,'C');
				$this->SetFont('SJIS','',9);
				$this->Cell(28.335,5,'’•¶“ú',0,0,'R');
				$this->SetFont('SJIS','',13);
				$this->Cell(38.335,6,date('y/m/d',strtotime($poIssueDate)),0,0,'R');
				
				$this->Ln();
				$this->SetFont('SJIS','',9);
				//~ $this->Cell(0,5,$this->PageNo().'/{nb} ƒy[ƒW',0,0,'R');
				$this->Cell(0,5,$this->GroupPageNo().'/'.$this->PageGroupAlias().' ƒy[ƒW',0,0,'R');
				...
				if($this->PageNo() == 1)
				{
					...
					$this->Cell(0,5,'Œä’†',0,0,'C');
					
					$this->SetY($this->GetY());
					$this->SetFont('SJIS','',10);
					$this->Cell(106.86,5,'',0,0,'R');
					$this->Cell(93.14,5,'ARK	ƒA[ƒNƒeƒbƒNŠ”Ž®‰ïŽÐ',0,0,'C');	$this->Ln();
					$this->Cell(106.86,5,'',0,0,'R');$this->SetFont('SJIS','',5.5);
					$this->Cell(93.14,5,'§321-0202@‰º“s‰êŒSp¶’¬‚¨‚à‚¿‚á‚Ì‚Ü‚¿4-11-16',0,0,'C');	$this->Ln();
					...
					$note = "‰º‹L‚Ì’Ê‚è’•¶‚¢‚½‚µ‚Ü‚·B\n’P‰¿–¢’è‚Í‘¬‚â‚©‚ÉŒ©Ï‘‚ð’ñoŠè‚¢‚Ü‚·\n”[“üêŠ‚ÍŒ´‘¥‚Æ‚µ‚Ä“–ŽÐ–{ŽÐ‚Æ‚·‚é\nŽx•¥•û–@“™‚ÍŒ»suŽx•¥•û–@“™‚É‚Â‚¢‚Äv‚É‚â‚è‚Ü‚·B\n–{’•¶‘‚Ì’P‰¿‚ÍÅ”²‚«‰¿Ši‚Å‚·B";
					...
					$this->Cell(25,5,'’•¶‡Œv‹àŠz','B',0,'L');
					$this->Cell(50,5,$totalAmount,'B',0,'C');
					$this->Cell(10,5,'(Å”²)',0,0,'C');
					$this->Ln(10);
					// -------------------------------------- END HEADER LEFT -------------------------------------- // 
				...
				$this->SetFont('SJIS','',9);
				$this->Cell(10,5,'NO','B',0,'L');
				//~ $this->Cell(23.75,5,'ƒƒbƒg”Ô†','B',0,'L');//Lot Number
				$this->Cell(23.75,5,'Lot No.','B',0,'L');//Lot Number
				$this->Cell(33.75,5,'}”Ô','B',0,'L');//Name
				$this->Cell(43.75,5,'•i–¼','B',0,'L');//Description
				$this->Cell(11.75,5,'’•¶”','B',0,'R');//Quantity
				$this->Cell(2,5,'','B',0,'R');
				$this->Cell(16.75,5,'Žx‹‹—\’è“ú','B',0,'C');
				$this->Cell(16.75,5,'”[Šú','B',0,'C');
	...
	if(count($lotNumberArray) > 0)
	{
		foreach($lotNumberArray as $lotNumber)
		{
			...
			if($queryLotList->num_rows > 0)
			{
				...
				if($poNo!='')
				{
					$sql = "SELECT productId, itemPrice, itemUnit FROM purchasing_pocontents WHERE poNumber LIKE '".$poNo."' AND lotNumber LIKE '".$lotNumber."'";
					$queryPoContent = $db->query($sql);
					if($queryPoContent->num_rows > 0)
					{
						while($resultPoContent = $queryPoContent->fetch_array())
						{
							...
							$itemUnit = $resultPoContent['itemUnit'];
				...
				$pdf->Cell(18.5,5,'”[Šú',0,0,'R');
				$pdf->Cell(18.5,5,$deliveryDate,0,0,'L');
				if($identifier==1)
				{				
					$pdf->Cell(18.5,5,'Žx‹‹—\’è“ú',0,0,'R');
	...
	{
		$attachmentFile = $_SERVER['DOCUMENT_ROOT']."/V3/4-9 Purchase Order Making Software/Email Attachment/".$poNumber."-1.pdf";
