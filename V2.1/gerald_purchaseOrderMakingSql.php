...
//~ print_r($_POST);
	if(isset($_POST['ajaxType']))
	{
		...
		else if($_POST['ajaxType']=='checkPONumber')
		{
			...
			if($_GET['country']!=2)
			{
				...
				if(!preg_match($pattern, $poNumber))
				{
	...
	else if(isset($_POST['finalize']))
	{
		...
		$notificationLink = '/'.v.'/4-9 Purchase Order Making Software/gerald_purchaseOrderStatus.php';
	...
	else if(isset($_GET['uniqueSupplier']))
	{
		...
		$notificationLink = '/'.v.'/4-9 Purchase Order Making Software/V2.1/gerald_purchaseOrderReview.php';
	...
	else if(isset($_POST['purchaseReviewConfirmation']))
	{
		...
		$sql = "
			SELECT a.id, a.processRemarks FROM view_workschedule as a
			INNER JOIN purchasing_forpurchaseorder as b ON b.lotNumber = a.lotNumber AND b.processRemarks = a.processRemarks
			WHERE a.processCode = 597 AND a.status = 0 AND a.lotNumber IN('".implode("','",$lotNumberArray)."') AND b.supplierType = 1
		";
		$queryWorkSchedule = $db->query($sql);
		if($queryWorkSchedule AND $queryWorkSchedule->num_rows > 0)
		{
	...
	else if(isset($_GET['purchaseReviewDelete']))
	{
		...
		$notificationLink = '/'.v.'/4-9 Purchase Order Making Software/V2.1/gerald_purchaseOrderMakingSql.php';
